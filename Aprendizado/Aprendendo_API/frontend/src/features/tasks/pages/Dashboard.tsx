    ...
    <div className="dashboard">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <Button onClick={() => setShowTrash(!showTrash)} variant="secondary" style={{ width: 'auto' }}>
        ...
        {showTrash ? (
          <>
            ...
            {deletedTasks.length === 0 ? (
              <p style={{ textAlign: 'center', color: '#94a3b8' }}>A lixeira está vazia.</p>
            ) : (
              deletedTasks.map(task => (
                <div key={task.id} className="task-item card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', padding: '15px', opacity: 0.7 }}>
                  <div style={{ flex: 1, paddingRight: '20px' }}>
                    <h4 style={{ marginBottom: '5px', fontSize: '18px', textDecoration: 'line-through' }}>{task.title}</h4>
                    {task.description && <p style={{ fontSize: '14px', color: '#cbd5e1' }}>{task.description}</p>}
                  </div>
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <Button onClick={() => handleRestore(task.id)} variant="primary" style={{ width: 'auto', padding: '5px 15px', fontSize: '12px' }}>
                      Restaurar
                    </Button>
                    <Button onClick={() => handlePermanentDelete(task.id)} variant="danger" style={{ width: 'auto', padding: '5px 15px', fontSize: '12px' }}>
                      Excluir Permanente
                    </Button>
                  </div>
                </div>
            ...
            {tasks.length === 0 ? (
              ...
              tasks.map(task => (
                <div key={task.id} className="task-item card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', padding: '15px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', flex: 1, paddingRight: '20px' }}>
                    <input 
                      type="checkbox" 
                      checked={task.completed} 
                      onChange={() => handleToggleComplete(task)}
                      style={{ marginRight: '15px', cursor: 'pointer', width: '20px', height: '20px' }}
                    />
                    <div>
                      <h4 style={{ 
                        marginBottom: '5px', 
